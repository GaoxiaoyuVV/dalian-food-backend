...
def login(request):
    if request.method == 'POST':
        ...
        try:
            ...
            obj = models.UserInfo.objects.filter(username=username, password = password).first()  # 只返回第一组值，之后就返回空
            if not obj:
                ret['code'] = 1001
                ret['msg'] = '用户名或密码错误'
...
def register(request):
    if request.method == 'POST':
        ...
        password = data['password']
        tel = data['tel']
        models.UserInfo.objects.create(username=user, password=password, tel=tel)
